...
from . import config
from .feature_extractors import FeatureExtractor, EncodecFeatures, MelSpectrogramFeatures
...
class Vocos(nn.Module):
    ...
    def make_denoising_vector(self):
        ...
        mel_bands = self.backbone.embed.in_channels
        mel_rand = torch.zeros((1, mel_bands, 88))
        bias_feats = self.backbone(mel_rand)
    ...
    @torch.inference_mode()
    def decode_mel(self, mel_spec, denoise=0.): # [B, bands=100, frames]           
        ...
        mag = mag - denoise*self.bias_vec.to(device=mag.device)
